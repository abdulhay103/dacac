import Image from "next/image";
import FounderImage from "/public/users/team-member.png";
import Link from "next/link";
import {
  FacebookLogo,
  InstagramLogo,
  LinkedinLogo,
  TwitterLogo,
} from "@phosphor-icons/react/dist/ssr";

export default function Therapists() {
  return (
    <div className="grid grid-flow-row grid-cols-12 items-center gap-6 lg:gap-8 xl:gap-14 pt-5 lg:pt-6 xl:pt-10">
      <div className="border bg-white border-cyan-500 rounded-md overflow-hidden shadow-lg col-span-12 lg:col-span-4 2xl:col-span-3 hover:scale-105 transition-all duration-500">
        <div className="px-4 xl:px-6 h-52 xl:h-60 overflow-hidden">
          <Image
            src={FounderImage}
            width="auto"
            height="auto"
            className=" object-cover"
            alt="Engr. Sobuj Rana"
          />
        </div>
        <div className="p-6">
          <h6 className=" font-semibold">Senior Physiotherapist</h6>
          <p>Razib Shaha</p>
          <p>Educational Qualification: B.Sc.</p>
        </div>
        <div className=" flex gap-6 justify-center py-3 bg-cyan-500">
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <FacebookLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <TwitterLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <LinkedinLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <InstagramLogo size={24} />
          </Link>
        </div>
      </div>
      <div className="border bg-white border-cyan-500 rounded-md overflow-hidden shadow-lg col-span-12 lg:col-span-4 2xl:col-span-3 hover:scale-105 transition-all duration-300">
        <div className="px-4 xl:px-6 h-52 xl:h-60 overflow-hidden">
          <Image
            src={FounderImage}
            width="auto"
            height="auto"
            className=" object-cover"
            alt="Engr. Sobuj Rana"
          />
        </div>
        <div className="p-6">
          <h6 className=" font-semibold">Senior Physiotherapist</h6>
          <p>Razib Shaha</p>
          <p>Educational Qualification: B.Sc.</p>
        </div>
        <div className=" flex gap-6 justify-center py-3 bg-cyan-500">
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <FacebookLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <TwitterLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <LinkedinLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <InstagramLogo size={24} />
          </Link>
        </div>
      </div>
      <div className="border bg-white border-cyan-500 rounded-md overflow-hidden shadow-lg col-span-12 lg:col-span-4 2xl:col-span-3 hover:scale-105 transition-all duration-300">
        <div className="px-4 xl:px-6 h-52 xl:h-60 overflow-hidden">
          <Image
            src={FounderImage}
            width="auto"
            height="auto"
            className=" object-cover"
            alt="Engr. Sobuj Rana"
          />
        </div>
        <div className="p-6">
          <h6 className=" font-semibold">Senior Physiotherapist</h6>
          <p>Razib Shaha</p>
          <p>Educational Qualification: B.Sc.</p>
        </div>
        <div className=" flex gap-6 justify-center py-3 bg-cyan-500">
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <FacebookLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <TwitterLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <LinkedinLogo size={24} />
          </Link>
          <Link
            href="#"
            className=" text-gray-100 hover:text-purple-500 transition-all hover:scale-105 hover:rotate-6"
          >
            <InstagramLogo size={24} />
          </Link>
        </div>
      </div>
    </div>
  );
}
